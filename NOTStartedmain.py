        ...
        if msg.media and hasattr(msg.media, 'poll'):
            ...
            for ans in poll.answers:
                if getattr(ans, 'correct', False):
                    ...
                    print(f"[DEBUG] Correct answer found: {correct_answer} — Question: {question}")
                    break

            if not correct_answer:
                print(f"[DEBUG] No correct answer defined — Question: {question}")
...
async def generate_txt(polls, event):
    ...
    for idx, (question, answers, correct) in enumerate(polls, 1):
        ...
        for ans in answers:
            ...
            output += f"{ans}{mark}\n"
        if not correct:
            output += "(No correct answer defined)\n"
        output += "\n"
